...
Exclude `.journal/`, credentials, private host identity, generated state,
caches, restored duplicates, and unmodified vendor trees. Put machine-specific
shell configuration in `~/.zshrc.local`. Put reviewed machine-specific desktop
values in a semantic host layer that does not expose a hostname.
